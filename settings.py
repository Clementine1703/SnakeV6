...
def get_setting_value_by_title(title, setting_objects):
    value = [obj['value']
             for obj in setting_objects if obj['title'] == title][0]
    return value


def get_setting_index_by_value(value, setting_objects):
    setting_object = [
        obj for obj in setting_objects if obj['value'] == value][0]
    index = setting_objects.index(setting_object)
    return index


data_for_generating_settings_fields = [
        {
            'type': 'combobox',
            'label_text': 'Размер карты',
            'setting_name': 'arena_size',
            'coordinates': {
                'lbl': (20, 60),
                'combobox': (110, 60),
            }
        ...
        {
            'type': 'combobox',
            'label_text': 'Счет для победы',
            'setting_name': 'score_winning',
            'coordinates': {
                'lbl': (20, 165),
                'combobox': (120, 165),
            }
        ...
        {
            'type': 'checkbutton',
            'label_text': 'Большое яблоко',
            'setting_name': 'apple_big',
            'coordinates': {
                'checkbutton': (20, 200)
            }
        },
        {
            'type': 'checkbutton',
            'label_text': 'Случ. размер яблока после съедания',
            'setting_name': 'apple_random_size',
            'coordinates': {
                'checkbutton': (20, 230)
            }
        },
